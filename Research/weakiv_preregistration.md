...
| 10 | 2026-08-25 (post-run, analysis time) | ar_accepts() tests beta in RAW units against STANDARDIZED blocks | ar_accepts omits the sy/sx rescale that ivestimators apply (G2 correction record 2), so for theta > 0 the tested null beta_std = 0.5 is FALSE in standardized units (true standardized coefficient = 0.5*sx/sy, e.g. 0.82 at theta = 0.88, alpha-invariant gamma). Consequence: ALL decisive-grid AR acceptance values decay artificially with signal strength (p = 1 coverage falls 0.95 -> 0.00 across theta at every alpha; p = 5 additionally inherits the broadcast null vector). INVALIDATED: ar_ok-derived quantities only (conditional coverages feeding X3). UNAFFECTED: X1 (null DGP, beta = 0), X2, X5 (theta = 0 by design => bug inactive), X6, and ALL of X4 (RMSE-based; f_pass/env_pass/kp flags are first-stage-only and remain valid). Discovered while reconciling full-grid conditional coverage; stream/statistic parity of the diagnosis proven bit-for-bit by regenerating stored ar_ok from seeds (scripts/recompute_ar_coverage.py --smoke). | No-silent-repairs rule; package stays pinned @ b4e10ed. |
| 11 | 2026-08-25 (operational) | Corrected-AR remediation dispatch | Corrected acceptance arrays ({cid}_arfix.npy) are regenerated from the registered streams with b_std = beta/(sy/sx) and combined with the VALID stored flag vectors: local tool `scripts/recompute_ar_coverage.py` (+ aggregator `scripts/aggregate_arfix.py`) ready for opportunistic local execution, and self-contained notebooks NBE1-NBE2 (~2.4 + ~2.0 predicted h) dispatched NOTEBOOK-first because the workstation was again under unrelated load (loadavg ~36 on 8 visible cores). Grids, seeds, thresholds unchanged. | Machine-etiquette rule; same registered replications, corrected measurement only. |
| 12 | 2026-08-25 (post-run, analysis time) | Second layer of the AR bug: H0 vector broadcast at p > 1 | The v1 corrected recompute fixed the units scaling but kept the broadcast null beta_std = (0.5,...,0.5); the true coefficient vector of make_single_spike is (0.5, 0, ..., 0) (y = 0.5*x1 + eps), so ALL p = 5 acceptance values - original AND v1-corrected - test a false joint null and sit near 0 coverage (v1 evidence: p = 5 cov ~ 0.00 vs p = 1 cov ~ 0.95 flat across theta). v2 fix tests b_orig = (0.5, 0, ..., 0)/resc; spot checks give 0.93-0.97 at p = 5 (60 reps). p = 1 values from the v1 NBE runs are VALID (single-column null identical in both parameterizations): unconditional coverage flat at ~0.95 across theta with genuine mild conditional selection effects (only 2 isolated conjunction cells, a0.3_k2_th0.72 and a0.9_k05_th0.88, no adjacent-alpha region among p = 1 designs). All v1 arfix arrays purged; NBE1-NBE2 regenerated with the correct vector and re-dispatched. | Falsifiable-progress rule: v1 outputs contradicted exactness theory at p = 5 and were traced to the second defect before any inference was drawn from them. |
